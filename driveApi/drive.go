...
func UploadFile(path string) error {
	...
	buf := bytes.NewBuffer(blackfriday.Run(file))
	name := strings.TrimSuffix(filepath.Base(path), filepath.Ext(path))

	drivefile := &drive.File{Name: name, MimeType: "application/vnd.google-apps.document"}
	_, err = srv.Files.Create(drivefile).Media(buf).Do()
